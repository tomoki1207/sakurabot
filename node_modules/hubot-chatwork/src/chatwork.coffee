...
class Chatwork extends Adapter
  ...
  run: ->
    options =
      ...
      ignore_accounts: process.env.HUBOT_CHATWORK_IGNORE_ACCOUNTS || ''
...
class ChatworkStreaming extends EventEmitter
  constructor: (options, @robot) ->
    ...
    @ignore_accounts = options.ignore_accounts.split ','
    if @ignore_accounts[0] == ''
      @ignore_accounts = []
        ...
        timeout = =>
          @Room(id).Messages().show (err, messages) =>
            for message in messages
              ...
              if @ignore_accounts.indexOf(message.account.account_id + '') >= 0
                continue
